...
const factory_support_1 = require("./core/factory-support");
...
(0, config_1.subtask)(constants_1.TASK_NODE_POLKADOT_CREATE_SERVER, "Creates a JSON-RPC server for Polkadot node")
    ...
    const server = (0, rpc_server_1.createRpcServer)({
        docker,
        nodePath,
        adapterPath,
...
(0, config_1.task)(task_names_1.TASK_TEST, async ({ testFiles, noCompile, parallel, bail, grep, }, { run, network, userConfig, config }, runSuper) => {
    ...
    if (network.config.polkavm !== true || network.name !== plugins_1.HARDHAT_NETWORK_NAME) {
        // If remote polkavm network
        if (network.config.polkavm)
            await (0, factory_support_1.handleFactoryDependencies)(config.paths.artifacts, network.config.url, network.config.polkadotUrl, network.config.accounts);
        return await runSuper();
    }
    ...
    try {
        ...
        await services_1.SubstrateNodeService.waitForNodeToBeReady(nodePort);
        await services_1.EthRpcService.waitForEthRpcToBeReady(adapterPort);
        await (0, utils_1.configureNetwork)(config, network, adapterPort || nodePort);
        let testFailures = 0;
        try {
            await (0, factory_support_1.handleFactoryDependencies)(config.paths.artifacts, `http://localhost:${adapterPort}`, `ws://localhost:${nodePort}`, constants_1.POLKADOT_NETWORK_ACCOUNTS);
