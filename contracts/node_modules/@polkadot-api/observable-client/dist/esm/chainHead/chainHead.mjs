...
import { Subject, filter, distinctUntilChanged, scan, mergeAll, map, switchMap, Observable, take, mergeMap, shareReplay, merge, share, ReplaySubject, tap, identity, noop, EMPTY, endWith, defer, of } from 'rxjs';
...
const getChainHead$ = (chainHead, getCachedMetadata, setCachedMetadata) => {
  ...
  const holdBlock = (blockHash, shouldThrow = false) => {
    ...
    let isPresent = false;
    started && pinnedBlocks$.pipe(take(1)).subscribe((blocks) => {
